...
void FProjectContextManager::RefreshContext()
{
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Refreshing project context..."));
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Project context gathered:"));
	UE_LOG(LogUnrealAICLI, Log, TEXT("  - Source files: %d"), CachedContext.SourceFiles.Num());
	UE_LOG(LogUnrealAICLI, Log, TEXT("  - UCLASS types: %d"), CachedContext.UClasses.Num());
	UE_LOG(LogUnrealAICLI, Log, TEXT("  - Level actors: %d"), CachedContext.LevelActors.Num());
	UE_LOG(LogUnrealAICLI, Log, TEXT("  - Total assets: %d"), CachedContext.AssetCount);
}
...
void FProjectContextManager::ScanSourceFiles()
{
	...
	{
		UE_LOG(LogUnrealAICLI, Warning, TEXT("Source directory not found: %s"), *SourceDir);
		return;
	}
...
bool FProjectContextManager::ParseSingleUClass(const FString& FileContent, const FString& RelativePath, int32 UClassPos, int32& OutNextSearchPos)
{
	...
	if (ClassPos == INDEX_NONE || ClassPos > UClassPos + UnrealAICLIConstants::Context::MaxUClassToClassKeywordDistance)
	{
	...
	if (InheritPos != INDEX_NONE && InheritPos < ClassNameEnd + UnrealAICLIConstants::Context::MaxClassNameToInheritanceDistance)
	{
		int32 ParentEnd;
...
FString FProjectContextManager::FormatContextForPrompt() const
{
	...
	if (CachedContext.UClasses.Num() > 0)
	{
		...
		int32 MaxClasses = FMath::Min(CachedContext.UClasses.Num(), UnrealAICLIConstants::Context::MaxClassesToFormat);
	...
	if (CachedContext.SourceFiles.Num() > 0)
	{
		...
		{
			if (DirCount++ >= UnrealAICLIConstants::Context::MaxDirectoriesToShow)
			{
				Context += FString::Printf(TEXT("  ... and %d more directories\n"), FilesByDir.Num() - UnrealAICLIConstants::Context::MaxDirectoriesToShow);
				break;
			}

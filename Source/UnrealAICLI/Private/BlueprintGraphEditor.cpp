	...
	FString& OutError)
{
	...
	if (NewNode)
	{
		...
		UE_LOG(LogUnrealAICLI, Log, TEXT("Created node '%s' (type: %s) at (%d, %d)"), *OutNodeId, *NodeType, PosX, PosY);
	}
...
bool FBlueprintGraphEditor::DeleteNode(UEdGraph* Graph, const FString& NodeId, FString& OutError)
{
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Deleted node '%s'"), *NodeId);
	return true;
}
	...
	FString& OutError)
{
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Connected '%s.%s' -> '%s.%s'"),
	...
	FString& OutError)
{
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Disconnected '%s.%s' from '%s.%s'"),
	...
	FString& OutError)
{
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Set pin '%s.%s' default value to '%s'"), *NodeId, *PinName, *Value);
	return true;
}

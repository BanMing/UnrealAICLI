...
FMCPToolResult FMCPTool_GetOutputLog::Execute(const TSharedRef<FJsonObject>& Params)
{
	...
	int32 NumLines = UnrealAICLIConstants::MCPServer::DefaultOutputLogLines;
	if (Params->HasField(TEXT("lines")))
	{
		NumLines = FMath::Clamp(static_cast<int32>(Params->GetNumberField(TEXT("lines"))), 1, UnrealAICLIConstants::MCPServer::MaxOutputLogLines);
	}

...
FClaudeSessionManager::FClaudeSessionManager()
	: MaxHistorySize(UnrealAICLIConstants::Session::MaxHistorySize)
{
}
...
{
	FString SaveDir = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealAICLI"));
...
bool FClaudeSessionManager::SaveSession()
{
	...
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to serialize session JSON"));
		return false;
	}

	if (!FFileHelper::SaveStringToFile(JsonString, *SessionPath, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM))
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to save session to: %s"), *SessionPath);
		return false;
	}

	UE_LOG(LogUnrealAICLI, Log, TEXT("Session saved to: %s (%d messages)"), *SessionPath, ConversationHistory.Num());
	return true;
}
...
bool FClaudeSessionManager::LoadSession()
{
	...
	{
		UE_LOG(LogUnrealAICLI, Log, TEXT("No previous session found at: %s"), *SessionPath);
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to load session from: %s"), *SessionPath);
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to parse session JSON"));
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAICLI, Log, TEXT("Session loaded from: %s (last updated: %s, %d messages)"),
			*SessionPath, *LastUpdated, ConversationHistory.Num());
	}
	else
	{
		UE_LOG(LogUnrealAICLI, Log, TEXT("Session loaded from: %s (%d messages)"),

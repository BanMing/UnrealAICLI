	...
	, ScriptCounter(0)
{
	...
	if (!IFileManager::Get().DirectoryExists(*ContentScriptDir))
	{
		...
		UE_LOG(LogUnrealAICLI, Log, TEXT("Created script directory: %s"), *ContentScriptDir);
	}
}
	...
	const FString& Description)
{
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("C++ script written to: %s"), *FilePath);
	...
	UE_LOG(LogUnrealAICLI, Warning, TEXT("C++ compilation failed, returning error for Claude to fix"));
...
bool FScriptExecutionManager::TriggerLiveCodingCompile(FString& OutErrorLog)
{
	...
	if (OutputCapture.bHasErrors)
	{
		...
		UE_LOG(LogUnrealAICLI, Error, TEXT("Live Coding compilation failed:\n%s"), *OutErrorLog);
		return false;
	}

	UE_LOG(LogUnrealAICLI, Log, TEXT("Live Coding compilation completed successfully"));
	return true;
#else
	...
	const FString& Description)
{
	...
	FUnrealAICLIOutputDevice ExecOutput;
	FUnrealAICLIOutputDevice LogOutput;
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Python script output (%d chars): %s"),
		Output.Len(), Output.Len() > 500 ? *(Output.Left(500) + TEXT("...")) : *Output);
	UE_LOG(LogUnrealAICLI, Log, TEXT("Python script actor delta: %d before, %d after (%+d)"),
	...
	const FString& Description)
{
	...
	FUnrealAICLIOutputDevice OutputDevice;
	FString AllOutput;
	...
	for (const FString& RawCommand : Commands)
	{
		...
		if (Command.Contains(TEXT("@UnrealAICLI")) || Command.Contains(TEXT("@Name:")) ||
	...
	const FString& ScriptName)
{
	...
	UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to write script to: %s"), *FilePath);
	return FString();
}
	...
	for (int32 i = 0; i < Recent.Num(); ++i)
	{
		...
		Context += FString::Printf(
			...
			*Entry.Description.Left(50),
            Entry.bSuccess ? TEXT("OK") : TEXT("FAIL")
		);
	}
...
bool FScriptExecutionManager::SaveHistory()
{
	...
	{
		UE_LOG(LogUnrealAICLI, Log, TEXT("Script history saved: %d entries"), History.Num());
		return true;
	}

	UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to save script history to: %s"), *HistoryPath);
	return false;
}
...
bool FScriptExecutionManager::LoadHistory()
{
	...
	{
		UE_LOG(LogUnrealAICLI, Log, TEXT("No script history file found"));
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to load script history from: %s"), *HistoryPath);
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to parse script history JSON"));
		return false;
	}
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Loaded script history: %d entries"), History.Num());
	return true;
}
...
{
	return FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealAICLI"), TEXT("script_history.json"));
}

FString FScriptExecutionManager::GetCppScriptDirectory() const
{
	// Source/[ProjectName]/Generated/UnrealAICLI/
	FString ProjectName = FApp::GetProjectName();
	return FPaths::Combine(FPaths::ProjectDir(), TEXT("Source"), ProjectName, TEXT("Generated"), TEXT("UnrealAICLI"));
}

...
FText SClaudeEditorWidget::GetStatusText() const
{
	if (bIsWaitingForResponse)
	{
		...
		FString StatusStr = FString::Printf(TEXT("¡ñ AI is thinking... %.1fs"), ElapsedSec);
	...
	if (!IsClaudeAvailable())
	{
		if (FClaudeCodeRunner::IsMCPOnlyMode())
		{
			return LOCTEXT("StatusMCPOnly", "¡ñ MCP-only mode");
		}
		return FText::FromString(FString::Printf(TEXT("¡ñ %s CLI not found"), *FClaudeCodeRunner::GetConfiguredProviderName()));
	}

	if (!LastResultStats.IsEmpty())
	{
		return FText::FromString(FString::Printf(TEXT("¡ñ %s"), *LastResultStats));
	}

	return LOCTEXT("StatusReady", "¡ñ Ready");
}
...
void SClaudeEditorWidget::UpdateToolGroupSummary()
{
	...
	if (ToolGroupCount == 1)
	{
		...
		if (ToolGroupDoneCount >= 1)
		{
			ToolGroupSummaryText->SetText(FText::FromString(
				FString::Printf(TEXT("âœ?%s completed"), *DisplayName)));
	...
	else
	{
		...
		{
			ToolGroupSummaryText->SetText(FText::FromString(
				FString::Printf(TEXT("âœ?%d tools completed"), ToolGroupCount)));
			ToolGroupSummaryText->SetColorAndOpacity(
				FSlateColor(FLinearColor(0.3f, 0.75f, 0.3f)));
		}
		else

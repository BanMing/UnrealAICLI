...
{
	FString ClaudePath = GetProviderPath(EUnrealAICLIProviderMode::Claude);
	return !ClaudePath.IsEmpty();
}

FString FClaudeCodeRunner::GetClaudePath()
{
	return GetProviderPath(EUnrealAICLIProviderMode::Claude);
}

EUnrealAICLIProviderMode FClaudeCodeRunner::GetProviderMode()
{
	FString ModeValue = TEXT("Claude");
	bool bHasProviderMode = false;

	if (GConfig)
	{
		// ProviderMode is the primary switch.
		bHasProviderMode = GConfig->GetString(TEXT("UnrealAICLI"), TEXT("ProviderMode"), ModeValue, GEditorIni);
	}
	...
	{
		return EUnrealAICLIProviderMode::Codex;
	}
	if (ModeValue.Equals(TEXT("Claude"), ESearchCase::IgnoreCase))
	{
		return EUnrealAICLIProviderMode::Claude;
	}

	// Compatibility: if ProviderMode is not configured, honor legacy bMCPOnlyMode.
	if (!bHasProviderMode && GConfig)
	{
		bool bLegacyMCPOnlyMode = false;
		if (GConfig->GetBool(TEXT("UnrealAICLI"), TEXT("bMCPOnlyMode"), bLegacyMCPOnlyMode, GEditorIni) && bLegacyMCPOnlyMode)
		{
			return EUnrealAICLIProviderMode::MCPOnly;
		}
	}
...
{
	return GetProviderMode() == EUnrealAICLIProviderMode::MCPOnly;
}

bool FClaudeCodeRunner::IsConfiguredProviderAvailable()
{
	EUnrealAICLIProviderMode Provider = GetProviderMode();
	if (Provider == EUnrealAICLIProviderMode::MCPOnly)
	{
		return false;
	}
...
FString FClaudeCodeRunner::GetProviderExecutableName(EUnrealAICLIProviderMode Provider)
{
	switch (Provider)
	{
...
FString FClaudeCodeRunner::GetProviderLabel(EUnrealAICLIProviderMode Provider)
{
	switch (Provider)
	{
	case EUnrealAICLIProviderMode::Codex:
...
FString FClaudeCodeRunner::GetProviderInstallHint(EUnrealAICLIProviderMode Provider)
{
	switch (Provider)
	{
	case EUnrealAICLIProviderMode::Codex:
...
FString FClaudeCodeRunner::GetProviderLoginHint(EUnrealAICLIProviderMode Provider)
{
	switch (Provider)
	{
	case EUnrealAICLIProviderMode::Codex:
...
FString FClaudeCodeRunner::GetProviderPath(EUnrealAICLIProviderMode Provider)
{
	if (Provider == EUnrealAICLIProviderMode::MCPOnly)
	{
		return FString();
	...
	for (const FString& Path : PossiblePaths)
	{
		...
		{
			UE_LOG(LogUnrealAICLI, Log, TEXT("Found %s CLI at: %s"), *GetProviderLabel(Provider), *Path);
	...
	if (FPlatformProcess::ExecProcess(WhichCmd, WhichArgs, &ReturnCode, &WhichOutput, &WhichErrors) && ReturnCode == 0)
	{
		...
		if (Lines.Num() > 0)
		{
			...
			UE_LOG(LogUnrealAICLI, Log, TEXT("Found %s CLI via '%s': %s"), *GetProviderLabel(Provider), WhichCmd, *Lines[0]);
			return Lines[0];
		}
	}

	UE_LOG(LogUnrealAICLI, Warning, TEXT("%s CLI not found. Install with: %s"), *GetProviderLabel(Provider), *GetProviderInstallHint(Provider));
	...
	FOnClaudeProgress OnProgress)
{
	...
	{
		UE_LOG(LogUnrealAICLI, Warning, TEXT("A provider request is already executing"));
		return false;
	}
...
bool FClaudeCodeRunner::ExecuteSync(const FClaudeRequestConfig& Config, FString& OutResponse)
{
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Executing %s: %s %s"), *GetConfiguredProviderName(), *ProviderPath, *CommandLine);
	...
	else
	{
		...
		UE_LOG(LogUnrealAICLI, Error, TEXT("Provider execution failed: %s"), *OutResponse);
		return false;
	}
}
...
static FString GetPluginDirectory()
{
	...
	FString EnginePluginPath = FPaths::Combine(FPaths::EnginePluginsDir(), TEXT("UnrealAICLI"));
	...
	FString MarketplacePluginPath = FPaths::Combine(FPaths::EnginePluginsDir(), TEXT("Marketplace"), TEXT("UnrealAICLI"));
	...
	// Try project plugins
	FString ProjectPluginPath = FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("UnrealAICLI"));
...
FString FClaudeCodeRunner::BuildCommandLine(const FClaudeRequestConfig& Config)
{
	...
	if (!PluginDir.IsEmpty())
	{
		...
		if (FPaths::FileExists(MCPBridgePath))
		{
			...
			FString MCPConfigDir = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealAICLI"));
			...
			if (FFileHelper::SaveStringToFile(MCPConfigContent, *MCPConfigPath))
			{
				...
				UE_LOG(LogUnrealAICLI, Log, TEXT("MCP config written to: %s"), *MCPConfigPath);
			}
			else
			{
				UE_LOG(LogUnrealAICLI, Warning, TEXT("Failed to write MCP config to: %s"), *MCPConfigPath);
			}
		}
		else
		{
			UE_LOG(LogUnrealAICLI, Warning, TEXT("MCP bridge not found at: %s"), *MCPBridgePath);
		}
	}
	...
	FString TempDir = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UnrealAICLI"));
	...
	if (!Config.SystemPrompt.IsEmpty())
	{
		...
		if (FFileHelper::SaveStringToFile(Config.SystemPrompt, *SystemPromptPath, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM))
		{
			...
			UE_LOG(LogUnrealAICLI, Log, TEXT("System prompt written to: %s (%d chars)"), *SystemPromptPath, Config.SystemPrompt.Len());
		}
	}
	...
	if (FFileHelper::SaveStringToFile(Config.Prompt, *PromptPath, FFileHelper::EEncodingOptions::ForceUTF8WithoutBOM))
	{
		...
		UE_LOG(LogUnrealAICLI, Log, TEXT("Prompt written to: %s (%d chars)"), *PromptPath, Config.Prompt.Len());
	}
...
FString FClaudeCodeRunner::BuildStreamJsonPayload(const FString& TextPrompt, const TArray<FString>& ImagePaths)
{
	...
	for (int32 i = 0; i < MaxCount; ++i)
	{
		...
		{
			UE_LOG(LogUnrealAICLI, Warning, TEXT("Rejecting image path with traversal: %s"), *FullImagePath);
			continue;
		}
		if (!FullImagePath.StartsWith(ExpectedDir))
		{
			UE_LOG(LogUnrealAICLI, Warning, TEXT("Rejecting image path outside screenshots directory: %s"), *FullImagePath);
			continue;
		}
		if (!FPaths::FileExists(FullImagePath))
		{
			UE_LOG(LogUnrealAICLI, Warning, TEXT("Attached image file no longer exists: %s"), *FullImagePath);
			continue;
		}
		...
		{
			UE_LOG(LogUnrealAICLI, Warning, TEXT("Failed to load image file for base64 encoding: %s"), *FullImagePath);
			continue;
		}
	...
	if (EncodedCount > 0)
	{
		UE_LOG(LogUnrealAICLI, Log, TEXT("Encoded %d image(s), total %lld bytes"), EncodedCount, TotalImageBytes);
	}
...
FString FClaudeCodeRunner::ParseStreamJsonOutput(const FString& RawOutput)
{
	...
	for (const FString& Line : Lines)
	{
		...
		if (Type == TEXT("result"))
		{
			...
			{
				UE_LOG(LogUnrealAICLI, Log, TEXT("Parsed stream-json result: %d chars"), ResultText.Len());
				return ResultText;
			}
		}
	}
	...
	{
		UE_LOG(LogUnrealAICLI, Log, TEXT("Parsed stream-json from assistant blocks: %d chars"), AccumulatedText.Len());
		return AccumulatedText;
	}

	// Last resort: return a user-friendly error instead of raw NDJSON
	UE_LOG(LogUnrealAICLI, Warning, TEXT("Failed to parse stream-json output (%d chars). Raw output logged below:"), RawOutput.Len());
	UE_LOG(LogUnrealAICLI, Warning, TEXT("%s"), *RawOutput.Left(2000));
...
void FClaudeCodeRunner::ParseAndEmitNdjsonLine(const FString& JsonLine)
{
	...
	if (!FJsonSerializer::Deserialize(Reader, JsonObj) || !JsonObj.IsValid())
	{
		UE_LOG(LogUnrealAICLI, Verbose, TEXT("NDJSON: Non-JSON line (skipping): %.200s"), *JsonLine);
		return;
	}
	...
	if (!JsonObj->TryGetStringField(TEXT("type"), Type))
	{
		UE_LOG(LogUnrealAICLI, Verbose, TEXT("NDJSON: Line missing 'type' field"));
		return;
	}

	UE_LOG(LogUnrealAICLI, Log, TEXT("NDJSON Event: type=%s"), *Type);
	...
	if (Type == TEXT("system"))
	{
		...
		UE_LOG(LogUnrealAICLI, Log, TEXT("NDJSON System: subtype=%s, session_id=%s"), *Subtype, *SessionId);
	...
	else if (Type == TEXT("assistant"))
	{
		...
		if (!JsonObj->TryGetObjectField(TEXT("message"), MessageObj))
		{
			UE_LOG(LogUnrealAICLI, Warning, TEXT("NDJSON: assistant message missing 'message' field"));
			return;
		}
		...
		if (!(*MessageObj)->TryGetArrayField(TEXT("content"), ContentArray))
		{
			UE_LOG(LogUnrealAICLI, Warning, TEXT("NDJSON: assistant message.content missing"));
			return;
		}
		...
		for (const TSharedPtr<FJsonValue>& ContentValue : *ContentArray)
		{
			...
			if (ContentType == TEXT("text"))
			{
				...
				if ((*ContentObj)->TryGetStringField(TEXT("text"), Text))
				{
					UE_LOG(LogUnrealAICLI, Log, TEXT("NDJSON TextContent: %d chars"), Text.Len());
			...
			{
				UE_LOG(LogUnrealAICLI, Verbose, TEXT("NDJSON: unknown content block type: %s"), *ContentType);
			}
		}
	}
	...
	{
		UE_LOG(LogUnrealAICLI, Verbose, TEXT("NDJSON: unhandled message type: %s"), *Type);
	}
}
...
bool FClaudeCodeRunner::CreateProcessPipes()
{
	...
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to create stdout pipe"));
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to create stdin pipe"));
		FPlatformProcess::ClosePipe(ReadPipe, WritePipe);
		ReadPipe = nullptr;
		WritePipe = nullptr;
		return false;
...
bool FClaudeCodeRunner::LaunchProcess(const FString& FullCommand, const FString& WorkingDir)
{
	...
	if (!ProcessHandle.IsValid())
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to create provider process"));
		UE_LOG(LogUnrealAICLI, Error, TEXT("Provider Path: %s"), *ProviderPath);
		UE_LOG(LogUnrealAICLI, Error, TEXT("Params: %s"), *Params);
		UE_LOG(LogUnrealAICLI, Error, TEXT("Working directory: %s"), *WorkingDir);
		return false;
	}
...
void FClaudeCodeRunner::ExecuteProcess()
{
	...
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("%s path no longer exists: %s"), *GetConfiguredProviderName(), *ProviderPath);
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Async executing %s: %s %s"), *GetConfiguredProviderName(), *ProviderPath, *CommandLine);
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Full command: %s %s"), *ProviderPath, *CommandLine);
	UE_LOG(LogUnrealAICLI, Log, TEXT("Working directory: %s"), *WorkingDir);

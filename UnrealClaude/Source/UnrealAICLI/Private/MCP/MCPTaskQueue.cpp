	...
	, LastCleanupTime(FDateTime::UtcNow())
{
	...
	if (!WakeUpEvent)
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to get sync event from pool for task queue"));
	}
}
...
void FMCPTaskQueue::Start()
{
	...
	if (WorkerThread)
	{
		UE_LOG(LogUnrealAICLI, Log, TEXT("MCP Task Queue started"));
	}
	else
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to create MCP Task Queue thread"));
	}
}
...
void FMCPTaskQueue::Shutdown()
{
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("MCP Task Queue shutting down..."));
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("MCP Task Queue stopped"));
}
...
FGuid FMCPTaskQueue::SubmitTask(const FString& ToolName, TSharedPtr<FJsonObject> Parameters, uint32 TimeoutMs)
{
	...
	{
		UE_LOG(LogUnrealAICLI, Warning, TEXT("Cannot submit task: Tool '%s' not found"), *ToolName);
		return FGuid();
	}
	...
	// Add to task map and queue
	{
		...
		{
			UE_LOG(LogUnrealAICLI, Warning, TEXT("Task queue at capacity (%d tasks), rejecting new task"), Config.MaxHistorySize);
			return FGuid();
		}
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Task submitted: %s (tool: %s)"), *Task->TaskId.ToString(), *ToolName);
...
bool FMCPTaskQueue::CancelTask(const FGuid& TaskId)
{
	...
	if (CurrentStatus == EMCPTaskStatus::Pending)
	{
		...
		UE_LOG(LogUnrealAICLI, Log, TEXT("Task cancelled (pending): %s"), *TaskId.ToString());
		return true;
	}
	else if (CurrentStatus == EMCPTaskStatus::Running)
	{
		...
		UE_LOG(LogUnrealAICLI, Log, TEXT("Task cancellation requested (running): %s"), *TaskId.ToString());
		return true;
	}
...
void FMCPTaskQueue::ExecuteTask(TSharedPtr<FMCPAsyncTask> Task)
{
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Task started: %s (tool: %s)"), *Task->TaskId.ToString(), *Task->ToolName);
	...
	else if (Task->TimeoutMs > 30000)
	{
		...
		{
			UE_LOG(LogUnrealAICLI, Error, TEXT("Task '%s' timed out after %d ms on game thread"),
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Task completed: %s (status: %s, duration: %.2fs)"),
...
void FMCPTaskQueue::CleanupOldTasks()
{
	...
	{
		UE_LOG(LogUnrealAICLI, Log, TEXT("Cleaned up %d old tasks"), TasksToRemove.Num());
	}
}
	...
	for (auto& Pair : Tasks)
	{
		...
		if (Task->Status.Load() == EMCPTaskStatus::Running)
		{
			...
			if (Elapsed.GetTotalMilliseconds() > Task->TimeoutMs)
			{
				...
				UE_LOG(LogUnrealAICLI, Warning, TEXT("Task timed out: %s"), *Task->TaskId.ToString());
			}
		}
	}
}

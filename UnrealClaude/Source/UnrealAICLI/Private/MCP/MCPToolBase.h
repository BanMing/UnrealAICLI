...
protected:
		...
		const FVector& DefaultValue = FVector::ZeroVector) const
	{
		...
		{
			return UnrealAICLIJsonUtils::ExtractVector(*VectorObj, DefaultValue);
		}
		return DefaultValue;
	}
		...
		const FRotator& DefaultValue = FRotator::ZeroRotator) const
	{
		...
		{
			return UnrealAICLIJsonUtils::ExtractRotator(*RotatorObj, DefaultValue);
		}
		return DefaultValue;
	}
	...
	TSharedPtr<FJsonObject> BuildActorInfoWithTransformJson(AActor* Actor) const
	{
		...
		if (Actor)
		{
			ActorJson->SetObjectField(TEXT("location"), UnrealAICLIJsonUtils::VectorToJson(Actor->GetActorLocation()));
			ActorJson->SetObjectField(TEXT("rotation"), UnrealAICLIJsonUtils::RotatorToJson(Actor->GetActorRotation()));
			ActorJson->SetObjectField(TEXT("scale"), UnrealAICLIJsonUtils::VectorToJson(Actor->GetActorScale3D()));
		}
		return ActorJson;
	}

...
{
	UE_LOG(LogUnrealAICLI, Log, TEXT("Registering MCP tools..."));
	...
	if (TSharedPtr<IMCPTool>* ExecuteScriptToolPtr = Tools.Find(TEXT("execute_script")))
	{
		if (FMCPTool_ExecuteScript* ExecuteScriptTool = static_cast<FMCPTool_ExecuteScript*>(ExecuteScriptToolPtr->Get()))
		{
			...
			UE_LOG(LogUnrealAICLI, Log, TEXT("  Wired up execute_script to task queue for async execution"));
		}
	}
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Registered %d MCP tools"), Tools.Num());
}
...
void FMCPToolRegistry::RegisterTool(TSharedPtr<IMCPTool> Tool)
{
	...
	{
		UE_LOG(LogUnrealAICLI, Warning, TEXT("Cannot register tool with empty name"));
		return;
	}

	if (Tools.Contains(Info.Name))
	{
		UE_LOG(LogUnrealAICLI, Warning, TEXT("Tool '%s' is already registered, replacing"), *Info.Name);
	}

	Tools.Add(Info.Name, Tool);
	UE_LOG(LogUnrealAICLI, Log, TEXT("  Registered tool: %s"), *Info.Name);
}
...
void FMCPToolRegistry::UnregisterTool(const FString& ToolName)
{
	...
	{
		InvalidateToolCache();
		UE_LOG(LogUnrealAICLI, Log, TEXT("Unregistered tool: %s"), *ToolName);
	}
}
...
FMCPToolResult FMCPToolRegistry::ExecuteTool(const FString& ToolName, const TSharedRef<FJsonObject>& Params)
{
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Executing MCP tool: %s"), *ToolName);
	...
	else
	{
		...
		const uint32 TimeoutMs = UnrealAICLIConstants::MCPServer::GameThreadTimeoutMs;
		const bool bSignaled = CompletionEvent->Wait(TimeoutMs);

		if (!bSignaled || !(*bTaskCompleted))
		{
			UE_LOG(LogUnrealAICLI, Error, TEXT("Tool '%s' execution timed out after %d ms"), *ToolName, TimeoutMs);
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("Tool '%s' execution %s: %s"),
		*ToolName,

...
FString FClipboardImageUtils::GetScreenshotDirectory()
{
	return FPaths::Combine(
		...
		TEXT("UnrealAICLI"),
		UnrealAICLIConstants::ClipboardImage::ScreenshotSubdirectory);
}
...
bool FClipboardImageUtils::SaveClipboardImageToFile(FString& OutFilePath, const FString& SaveDirectory)
{
	...
	{
		UE_LOG(LogUnrealAICLI, Warning, TEXT("Failed to open clipboard"));
		return false;
	}
	...
	{
		::CloseClipboard();
		UE_LOG(LogUnrealAICLI, Warning, TEXT("No DIB data in clipboard"));
		return false;
	}
	...
	{
		::CloseClipboard();
		UE_LOG(LogUnrealAICLI, Warning, TEXT("Failed to lock clipboard data"));
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAICLI, Warning, TEXT("Unsupported clipboard image format: %dx%d, %d bpp"), Width, Height, BitCount);
		::GlobalUnlock(hData);
		::CloseClipboard();
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAICLI, Warning, TEXT("Clipboard image dimensions too large: %dx%d (max %d)"), Width, Height, MaxReasonableDimension);
		::GlobalUnlock(hData);
		::CloseClipboard();
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAICLI, Warning, TEXT("Unsupported clipboard DIB compression type: %d"), Header->biCompression);
		::GlobalUnlock(hData);
		::CloseClipboard();
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to create PNG image wrapper"));
		return false;
	}

	if (!ImageWrapper->SetRaw(Pixels.GetData(), Pixels.Num() * sizeof(FColor), Width, Height, ERGBFormat::BGRA, 8))
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to set raw pixel data for PNG compression"));
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("PNG compression returned empty data"));
		return false;
	}
	...
	{
		UE_LOG(LogUnrealAICLI, Error, TEXT("Failed to save clipboard image to: %s"), *OutFilePath);
		return false;
	}

	UE_LOG(LogUnrealAICLI, Log, TEXT("Saved clipboard image: %s (%dx%d, %lld bytes)"),
		...
		&ReturnCode, &StdOut, &StdErr) && ReturnCode == 0)
	{
		...
		{
			UE_LOG(LogUnrealAICLI, Log, TEXT("Saved clipboard image via wl-paste: %s (%lld bytes)"),
		...
		&ReturnCode, &StdOut, &StdErr) && ReturnCode == 0)
	{
		if (IFileManager::Get().FileExists(*OutFilePath) && IFileManager::Get().FileSize(*OutFilePath) > 0)
		{
			UE_LOG(LogUnrealAICLI, Log, TEXT("Saved clipboard image via xclip: %s (%lld bytes)"),
				*OutFilePath, IFileManager::Get().FileSize(*OutFilePath));
			return true;
		}
	}
	...
	UE_LOG(LogUnrealAICLI, Warning, TEXT("Failed to get clipboard image. Install wl-paste (wl-clipboard) or xclip."));
	return false;
	...
	UE_LOG(LogUnrealAICLI, Log, TEXT("SaveClipboardImage: saving to %s"), *OutFilePath);
	...
	if (FFileHelper::SaveArrayToFile(ScriptBytes, *ScriptPath))
	{
		...
		if (FPlatformProcess::ExecProcess(TEXT("/usr/bin/osascript"), *ScriptPath, &ReturnCode, &StdOut, &StdErr) && ReturnCode == 0)
		{
			...
			if (IFileManager::Get().FileExists(*OutFilePath) && IFileManager::Get().FileSize(*OutFilePath) > 0)
			{
				UE_LOG(LogUnrealAICLI, Log, TEXT("Saved clipboard image via osascript: %s (%lld bytes)"),
					*OutFilePath, IFileManager::Get().FileSize(*OutFilePath));
				return true;
			}
		}
		UE_LOG(LogUnrealAICLI, Warning, TEXT("SaveClipboardImage: osascript failed (ReturnCode=%d, StdErr='%s')"),
			ReturnCode, *StdErr);
		IFileManager::Get().Delete(*ScriptPath);
	}
	...
	UE_LOG(LogUnrealAICLI, Warning, TEXT("Failed to get clipboard image on macOS"));
	return false;

#else
	UE_LOG(LogUnrealAICLI, Warning, TEXT("Clipboard image paste is not supported on this platform"));
	return false;
#endif
}
...
void FClipboardImageUtils::CleanupOldScreenshots(const FString& Directory, double MaxAgeSeconds)
{
	...
	if (DeletedCount > 0)
	{
		UE_LOG(LogUnrealAICLI, Log, TEXT("Cleaned up %d old clipboard screenshots from %s"), DeletedCount, *Directory);
	}
}

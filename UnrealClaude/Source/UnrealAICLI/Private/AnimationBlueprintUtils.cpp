	...
	FString& OutError)
{
	...
	for (int32 RuleIndex = 0; RuleIndex < Rules.Num(); RuleIndex++)
	{
		...
		for (UAnimStateTransitionNode* Transition : AllTransitions)
		{
			...
			int32 PosX = UnrealAICLIConstants::AnimDiagram::ConditionNodeStartX;
			int32 PosY = UnrealAICLIConstants::AnimDiagram::ConditionNodeStartY;
				...
				if (CondType.Equals(TEXT("TimeRemaining"), ESearchCase::IgnoreCase))
				{
					...
					else
					{
						...
						UEdGraphNode* CompNode = AddConditionNode(
							AnimBP, StateMachineName, FromStateName, ToStateName,
							Comparison, nullptr, FVector2D(PosX + UnrealAICLIConstants::AnimDiagram::ConditionNodeSpacing, PosY),
	...
	const TArray<FDiagramTransition>& Transitions)
{
	...
	const int32 CellWidth = UnrealAICLIConstants::AnimDiagram::DiagramCellWidth;
	const int32 CellHeight = UnrealAICLIConstants::AnimDiagram::DiagramCellHeight;
	...
	for (int32 y = 0; y <= MaxY; y++)
	{
		...
		for (int32 x = 0; x <= MaxX; x++)
		{
			if (const FDiagramState* const* StatePtr = StateGrid.Find(TPair<int32, int32>(x, y)))
			{
				...
				if (StateName.Len() > UnrealAICLIConstants::AnimDiagram::MaxStateNameDisplayLength)
				{
					StateName = StateName.Left(UnrealAICLIConstants::AnimDiagram::MaxStateNameDisplayLength - 1) + TEXT(".");
				}
